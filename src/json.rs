...
/// Convert an rlua `Value` to a `serde_json::Value` for encoding.
fn lua_to_json(lua: &rlua::Lua, val: rlua::Value) -> Result<serde_json::Value, String> {
    match val {
        rlua::Value::Nil => Ok(serde_json::Value::Null),
        rlua::Value::Boolean(b) => Ok(serde_json::Value::Bool(b)),
        rlua::Value::Integer(i) => Ok(serde_json::Value::Number(
            serde_json::Number::from(i),
        )),
        rlua::Value::Number(n) => {
            // rlua::Number is f64. Only serialize if it's finite.
            if !n.is_finite() {
        ...
        rlua::Value::String(s) => Ok(serde_json::Value::String(
...
/// Convert a `serde_json::Value` to an rlua `Value` for decoding.
fn json_to_lua<'lua>(
    lua: &'lua rlua::Lua,
...
pub fn encode(lua: &rlua::Lua, val: rlua::Value) -> Result<String, String> {
    let json = lua_to_json(lua, val)?;
    serde_json::to_string(&json).map_err(|e| e.to_string())
}

/// Encode a Lua value to a pretty-printed JSON string.
pub fn encode_pretty(lua: &rlua::Lua, val: rlua::Value) -> Result<String, String> {
    let json = lua_to_json(lua, val)?;
    serde_json::to_string_pretty(&json).map_err(|e| e.to_string())
}

/// Decode a JSON string into a Lua value.
pub fn decode<'lua>(lua: &'lua rlua::Lua, text: String) -> Result<rlua::Value<'lua>, String> {
